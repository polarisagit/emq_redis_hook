...
all() -> 
    [{group, emq_redis_hook_actions},
     {group, emq_redis_hook}
     ].

groups() -> 
    [{emq_redis_hook, [sequence], [reload, server_config, change_config]},
     {emq_redis_hook_actions, [sequence], [case1]}
    ].

init_per_suite(Config) ->
    DataDir = proplists:get_value(data_dir, Config),
    [start_apps(App, DataDir) || App <- [emqttd, emq_redis_hook]],
    start_http_(),
    Config.

end_per_suite(_Config) ->
    mochiweb:stop_http(8080),
    [application:stop(App) || App <- [emq_redis_hook, emqttd]].

reload(_Config) -> 
    {ok, Rules} = application:get_env(emq_redis_hook, rules),
...
server_config(_) ->
    ...
    {ok, Url} =  application:get_env(emq_redis_hook, url),
...
change_config(_Config) ->
    ...
    application:set_env(emq_redis_hook, rules, HookRules),
    emq_redis_hook:load(),
    ?assertEqual([], ?HOOK_LOOKUP("message.delivered")),
    emq_redis_hook:unload(),
    application:set_env(emq_redis_hook, rules, Rules),
    emq_redis_hook:load().
